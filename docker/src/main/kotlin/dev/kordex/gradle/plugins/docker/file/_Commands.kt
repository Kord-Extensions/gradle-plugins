...
import dev.kordex.gradle.plugins.docker.annotations.DockerfileCommandDsl
...
@DockerfileCommandDsl
fun CommandList.env(body: EnvBuilder.() -> Unit) {
	val builder = EnvBuilder()

	body(builder)

	add(EnvCommand(builder.variables))
}
...
@DockerfileCommandDsl
fun CommandList.shell(vararg command: String) {
	add(ShellCommand(command.toList().toTypedArray()))
}
...
@DockerfileCommandDsl
fun CommandList.volume(vararg volumes: String) {
	add(VolumeCommand(volumes.toList().toTypedArray()))
}
